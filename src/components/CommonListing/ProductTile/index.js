...
export default function ProductTile({ item }) {
  ...
  return (
    <div  onClick={()=> router.push(`/product/${item._id}`)}>
      ...
      {item.onSale === "yes" ? (
        <div className="absolute top-0 m-2 rounded-full bg-red-700 hover:bg-pink-400 transition">
      ...
      <div className="my-4 mx-auto flex w-10/12 flex-col items-start justify-between">
        <div className="mb-2 flex">
          ...
          {item.onSale === "yes" ? (
            <p className="mr-3 text-sm font-semibold text-red-700 hover:text-pink-400 transition">{`Rs ${(
              item.price -
